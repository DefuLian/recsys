...
a = [metric_spatial_cv150, metric_geo_cv, metric_graph_cv100, metric_cv, metric_hpf, metric_ucf,metric_bpr];
...
legend('GeoMF++', 'GeoMF', 'GWMF', 'WRMF', 'HPF', 'UCF', 'BPRMF', 'location', 'southeast')
...
%set(gca, 'xtick',5:5:20);
set(gca, 'xticklabel',0:50:200);
legend('GeoMF++', 'GeoMF', 'GWMF', 'WRMF','HPF', 'UCF', 'BPRMF', 'location', 'northwest')
...
linespec = {'-.x', '-.x','--.',':+','-o'};
for i=1:size(time,2)
    semilogy(time(:, i), linespec{i}, 'linewidth',2); hold on
end
ApplyFigTemplate(gcf,gca);
...
load(sprintf('~/data/checkin/%s/sensitive.mat', dataset));
%
a = [metric_K_piccf1{9}, metric_K_geomf{3}, metric_graph_cv, metric_cv150, metric_hpf, metric_ucf, metric_bpr];
