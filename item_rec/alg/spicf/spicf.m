...
[P, Q] = piccf(R, 'alpha', alpha, 'max_iter', max_iter, 'K', K, 'reg_u', reg_u, 'reg_i', reg_i, 'usr_w', a, 'item_w', d);
pace = initial_pace(R, P, Q, a, d);
for i=1:10
    %P = randn(M, K) * 0.01; Q = randn(N, K) * 0.01;
    eval = evaluate_item(R, test, P, Q, 200, 200);
    fprintf('pace=%f, ndcg@50=%f\n', pace, eval.ndcg(50))
    [a, d] = optimize_theta_beta(R, P, Q, a, d, pace, is_hard);
    pace = pace * mu;
    save(sprintf('pq%d.mat',i), 'P', 'Q'); 
    [P, Q] = piccf(R, 'alpha', alpha, 'max_iter', max_iter, 'K', K, 'reg_u', reg_u, 'reg_i', reg_i,...
