...
for iter=1:max_iter
    ...
    fprintf('Iteration=%d of all optimization, loss=%f', iter, Loss(R, W, P, Q, usr_w, item_w));
...
QtQ = Q.' * spdiags(d, 0, length(d), length(d)) * Q;
[~, M] = size(W);
Qt = Q.';
bR = Qt * (bI .* d);
for i = 1 : M
...
if F >0
    if F < 20000 || nnz(X)<0.001 * numel(X)
        ...
        [U, v] = ConjGrad(X, P, reg, 'X0', U, 'tol', 1e-6);
    end
end
end
